...
Проженіть перевірку постобробки без моделі та камери:

```bash
python checks/check_lab.py --target solution
```

Потім запустіть детекцію на зображенні й заміряйте FPS після прогріву.

## Розбір збоїв

- Модель не знайдено — експортуйте `yolov8n.onnx`
- NMS отримує xyxy замість xywh — рамки зникають
- Перший інференс повільний — модель не прогріта
