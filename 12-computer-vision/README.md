...
Детектор із чистою постобробкою під тестами (`solution/detection.py`) і виміряний FPS (крок 6 у `lab.md`).

## Зв'язок з capstone

Розділ «Що далі» в `capstone/README.md`: CV-сервіс підключається до capstone як споживач відео та публікує детекції в чергу.
