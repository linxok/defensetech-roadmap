# 14. Наземна станція управління

## GCS

Ground Control Station — інтерфейс оператора для моніторингу, планування місій, керування дроном. Сучасні GCS — це веб-додатки з картами, WebSocket і відео.

## Технології

- **React** — UI.
- **Next.js** — full-stack фреймворк.
- **Leaflet / MapLibre** — мапи.
- **WebSocket** — live telemetry.
- **WebRTC** — low-latency video.

## Функції GCS

- Live map з позицією дронів.
- Telemetry panels (battery, altitude, speed, GPS).
- Mission planning (waypoints, geofence).
- Alerts (low battery, failsafe, no GPS).
- Video stream.
...
React + WebSocket + Leaflet GCS prototype.
...
Модуль 14 не існує ізольовано. Його знання поєднуються з попередніми і наступними модулями. Наприклад, Linux і мережі використовуються в MAVLink, Python/C++ — для backend, ROS2 і CV — для AI, а DevOps — для розгортання. Курс побудований так, щоб кожен модуль доповнював загальну картину.
