...
Перевірте офлайн-режим і валідацію:

```bash
python checks/check_lab.py --target solution
```

Без `OPENAI_API_KEY` має працювати детермінований генератор; невалідна місія — відхилятися з поясненням.

## Розбір збоїв

- Немає fallback — сервіс непрацездатний без API
- JSON від моделі не валідується — сміття потрапляє в автопілот
- Ключ у коді — витік секрету
