...
## 10 проєктів для портфоліо

1. **Telemetry Service** — збір, збереження і роздача телеметрії.
2. **Mission Service** — API для планування і виконання місій.
3. **Fleet Manager** — управління флотом дронів.
4. **Ground Control UI** — веб-GCS з картою і відео.
5. **MAVLink Gateway** — шлюз між дроном і backend.
6. **AI Detection Service** — детекція об’єктів у відеопотоці.
7. **Video Analytics** — запис, пошук, аналітика відео.
8. **Flight Log Analyzer** — парсинг і візуалізація польотних журналів.
9. **Drone AI Agent** — LLM/VLM агент для планування.
10. **Defense Dashboard** — єдиний dashboard для оператора.

## Як обирати

- Почніть з Telemetry Service і MAVLink Gateway.
- Додайте GCS для демонстрації end-to-end системи.
- Потім — AI Detection та Flight Log Analyzer для AI/ML напрямку.

## Рекомендації

- Кожен проєкт у окремому репозиторії.
- README, Docker, CI/CD.
- Демо-скріншоти або коротке відео.
- Покрийте код тестами.
...
Модуль 16 не існує ізольовано. Його знання поєднуються з попередніми і наступними модулями. Наприклад, Linux і мережі використовуються в MAVLink, Python/C++ — для backend, ROS2 і CV — для AI, а DevOps — для розгортання. Курс побудований так, щоб кожен модуль доповнював загальну картину.
