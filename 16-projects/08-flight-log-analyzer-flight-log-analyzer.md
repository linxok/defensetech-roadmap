# Проєкт 08-flight-log-analyzer: Flight Log Analyzer

## Опис

Парсинг, візуалізація та аналіз польотних журналів.

## Функціональність

- ULog/.bin parser
- Time-series charts
- Anomaly detection
- Report generation
- Compare flights
- Export data

## Архітектура

```mermaid
graph LR
    A[Drone] --> B[Gateway]
    B --> C[Queue]
    C --> D[Workers]
    D --> E[Database]
    E --> F[API]
    F --> G[Dashboard]
```

## Технологічний стек

- Python / FastAPI або Node.js / NestJS
- PostgreSQL / Redis
- RabbitMQ / Kafka
- Docker / Kubernetes
- React / Next.js / Leaflet
- Prometheus / Grafana

## Етапи реалізації

1. Проєктування API та схеми даних.
2. Реалізація core функціоналу.
3. Інтеграція з дроном / SITL.
4. Тестування та дебаг.
5. Документація, Docker, CI/CD.
6. Деплой та демо.

## Критерії готовності

- [ ] Код у публічному репозиторії.
- [ ] README з інструкцією запуску.
- [ ] Docker Compose або Kubernetes маніфести.
- [ ] CI/CD pipeline.
- [ ] Тести або чекліст якості.
- [ ] Демо: скріншот, відео або live URL.

## Критерії приймання (DoD)

- [ ] Звіт містить 5+ метрик з одиницями вимірювання
- [ ] Аномалії детектуються за порогами і мають часові мітки
- [ ] Битий/обрізаний лог не валить аналізатор
- [ ] Експорт звіту в Markdown/JSON перевірено на тестовому лозі
- [ ] Час аналізу одного логу < 1 хвилини

## Стартовий код

- `08-px4/examples/ulog_summary.py` — читання ULog топіків
- `08-px4/solution/px4_param.py` — робота з параметрами

## Рекомендації

- Починайте з мінімального viable продукту.
- Використовуйте SITL для тестування без реального дрона.
- Додайте логування та моніторинг з першого дня.
- Документуйте API з OpenAPI/Swagger.
- Покрийте код тестами поступово.

## Складність

**Рівень:** Intermediate — Advanced
**Час:** 2–6 тижнів залежно від глибини.
