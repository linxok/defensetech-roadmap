...
- Міграції (Alembic) обов’язкові з першого дня: схема в коді, а не «руками в psql».
- Спостережуваність: `/health`, `/metrics`, структурні логи з `drone_id` і `trace_id`; без цього інциденти не розслідуються.
...
Крок 10: capstone використовує саме цей стек — сервіси `backend`, `worker`, `postgres`, `rabbitmq`.
